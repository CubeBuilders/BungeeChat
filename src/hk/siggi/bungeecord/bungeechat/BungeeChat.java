...
public class BungeeChat extends Plugin implements Listener, VariableServerConnection.Listener {
	...
	public TextComponent unify(List<BaseComponent> components) {
	...
	public ArrayList<BaseComponent> processChat(ProxiedPlayer sender, String text) {
		return processChat(sender, text, false);
	}

	private ArrayList<BaseComponent> processChat(ProxiedPlayer sender, String text, boolean isInsideLink) {
					...
					if ((link.startsWith("/") || link.contains(":")) && ((!link.startsWith("/") || allowCommandLinks) && (lowerLink.startsWith("http://") || lowerLink.startsWith("https://") || allowNonWebLinks))) {
						...
						if (startTextPos < endTextPos) {
							...
							for (BaseComponent c : linkComponents) {
								...
								if (link.startsWith("/")) {
									c.setHoverEvent(new HoverEvent(HoverEvent.Action.SHOW_TEXT, new BaseComponent[]{
										new TextComponent(link)
									}));
									if (link.endsWith("...")) {
									...
									c.setHoverEvent(new HoverEvent(HoverEvent.Action.SHOW_TEXT, new BaseComponent[]{
										new TextComponent("This link is blacklisted")
									}));
								} else {
									c.setHoverEvent(new HoverEvent(HoverEvent.Action.SHOW_TEXT, new BaseComponent[]{
										new TextComponent(link)
							...
							if (link.startsWith("/")) {
								linkTextComponent.setHoverEvent(new HoverEvent(HoverEvent.Action.SHOW_TEXT, new BaseComponent[]{
									new TextComponent("Click to run command")
								}));
								if (link.endsWith("...")) {
								...
								if (sender != null && Util.isChatLinkWhitelisted(link) && (title = Util.getTitle(link)) != null) {
									...
									linkTextComponent.setHoverEvent(new HoverEvent(HoverEvent.Action.SHOW_TEXT, new BaseComponent[]{
										new TextComponent("Click to visit page")
									}));
									linkTextComponent.setClickEvent(new ClickEvent(ClickEvent.Action.OPEN_URL, link));
								}
							}
	...
	private boolean processChatLine(String line) {
		...
		/*int pos = line.indexOf(":");
		 ...
		 String chatType = line.substring(0, pos);
		 line = line.substring(pos + 1);
		 if (chatType.startsWith("Public-")) {
		 String serverKind = chatType.substring(7);
		 pos = line.indexOf(":");
		 if (pos != -1) {
		 String from = line.substring(0, pos);
		 line = line.substring(pos + 1);
		 sendPublicSpy(serverKind, from, line);
		 return true;
		 }
		 }
		 if (chatType.startsWith("Faction-")) {
		 String serverKind = chatType.substring(8);
		 pos = line.indexOf(":");
		 if (pos != -1) {
		 String from = line.substring(0, pos);
		 line = line.substring(pos + 1);
		 pos = line.indexOf(":");
		 if (pos != -1) {
		 String to_ = line.substring(0, pos);
		 line = line.substring(pos + 1);
		 String[] to;
		 if (to_.length() == 0) {
		 to = new String[0];
		 ...
		 sendFactionSpy(serverKind, from, to, line);
		 return true;
		 }
		 }
		 }
		 ...
		 line = line.substring(pos + 1);
		 sendPrivateSpy(from, to, line);
		 if (doNotLog.contains(from.toLowerCase()) || doNotLog.contains(to.toLowerCase())) {
		 return false;
		 }
		 return true;
		 }
