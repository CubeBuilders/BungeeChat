...
public class CommandMute extends Command implements TabExecutor {

	public final BungeeChat plugin;

	public CommandMute(BungeeChat plugin) {
		super("mute", null);
		this.plugin = plugin;
	}
		...
		if (receiverUUID == null) {
			...
			for (int i = 0; i < players.length; i++) {
				...
				if (matcher.matches()) {
					...
					BaseComponent message = new TextComponent("Issue mute to " + receiverPlayer.getName() + "? ");
